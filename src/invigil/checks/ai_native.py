...
# Match these as *imports*, never as bare substrings. `autogen` is also the stem of
# "autogenerated", which pip-compile and Sphinx write into files in most Python
# repos — matching that told aws/aws-cli it was building agents.
_PKGS = "|".join(AGENT_PACKAGES)
AGENT_IMPORT = re.compile(
    rf"^[ \t]*(?:from[ \t]+({_PKGS})[.\s]|import[ \t]+({_PKGS})\b)"  # python
    rf"|(?:from|require\()[ \t]*['\"]({_PKGS})(?:['\"/])",  # js / ts
    re.MULTILINE,
)
...
def agent_scope_visibility(ctx: Context) -> CheckResult:
    ...
    uses_agents = ctx.source_matches(AGENT_IMPORT, suffixes=(".py", ".ts", ".js"))
    if not uses_agents:
