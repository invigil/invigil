...
# Regression: the markers were matched as bare substrings, and `autogen` is the
# stem of "autogenerated" — a word pip-compile and Sphinx write into most Python
# repos. aws/aws-cli was told it "uses an agent framework" on the strength of
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "mcpath = 'x'\nlangchainish = 2\n",  # names that merely start with a package
    ],
)
def test_agent_scope_ignores_incidental_word_matches(tmp_path, body):
    (tmp_path / "conf.py").write_text(body)
    assert ai.agent_scope_visibility(ctx(tmp_path)).status is Status.SKIP
...
def test_agent_scope_still_detects_real_imports(tmp_path, body):
    (tmp_path / "agent.py").write_text(body)
    assert ai.agent_scope_visibility(ctx(tmp_path)).status is not Status.SKIP


def test_agent_scope_detects_js_imports(tmp_path):
    (tmp_path / "agent.js").write_text('import { ChatOpenAI } from "langchain/chat_models";\n')
    assert ai.agent_scope_visibility(ctx(tmp_path)).status is not Status.SKIP
